  ...
  function getMangers() {
    if (grouped) {
      ...
      return Object.keys(groups).map(groupId => ({
          ...
          items: Object.values(AMOCRM.constant('managers'))
            .filter(item => String(item.group) === String(groupId))
            .map(user => ({
            id: +user.id,
            value: user.title
          }))
      ...
      return Object.values(AMOCRM.constant('managers')).map(user => ({
        id: +user.id,
        value: user.title
      }));
    }
